...
public class StepDetector{
	...
	public boolean FindStep(){
		...
		this.dataCurr = this.Data.get(this.Data.size()-1).getValue(this.stepAxis);
	...
	public void findPeak(){
		...
		if (this.peakValue == 0){
			...
			while (j < this.LOOKLENGTH){
				Prev = this.Data.get(this.Data.size()-1-j).getValue(this.stepAxis);
				if (Prev > this.dataCurr)
					break; //not a true peak
				j++;
			}
			if (j == this.LOOKLENGTH && Math.abs(this.dataCurr - this.peakAvg) < this.PEAKAVGTHRESH){
	...
	public boolean verifyPeak(){
		...
		if (this.peakValue != 0 && this.peakVerified == false && this.peakLookCounter < this.LOOKLENGTH){
	...
	public void findTrough(){
		if (this.troughValue == 0){
			...
			double Prev = this.Data.get(this.Data.size()-2).getValue(this.stepAxis);
			long PrevTS = this.Data.get(this.Data.size()-2).getTimestamp();
			if(this.dataCurr > Prev){ //started going back up = possible trough
	...
	public boolean verifyTrough(){
		...
		if (this.troughValue != 0 && this.troughVerified == false && this.troughLookCounter < this.LOOKLENGTH){
